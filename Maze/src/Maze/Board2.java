...
public class Board2 extends JPanel implements ActionListener{
	...
	public void paint(Graphics g) {
		...
		if (!win) {
			...
			g.drawImage(p.getGozluklu(), p.getTileX() * 32, p.getTileY() * 32, null);
	...
	public class Al2 extends KeyAdapter {
		public void keyPressed(KeyEvent e) {
			...
			if (keycode == KeyEvent.VK_W) {
				if (!m.getMap(p.getTileX(), p.getTileY() - 2).equals("0")&&
						!m.getMap(p.getTileX(), p.getTileY() - 1).equals("0")) {
					p.move(0, -2);
				}
			}
			...
			if (keycode == KeyEvent.VK_A) {
				if (!m.getMap(p.getTileX() - 2, p.getTileY()).equals("0")&&
						!m.getMap(p.getTileX() - 1, p.getTileY()).equals("0")) {
					p.move(-2, 0);
				}
			}
